...
# 数据参数
data_path = 'exp/datasets/ood_msi/1.mat'
image_size = 512
channels = 31
...
# 数据保存参数
save_dir = deg + str(deg_scale) + '_DHP'
...
# 保存为RGB图像
plt.imsave('experiment/'+save_dir+'/Apy.png', to_rgb(inverse_data_transform(Apy).squeeze().permute(1, 2, 0).detach().cpu().numpy()))
plt.imsave('experiment/'+save_dir+'/orig.png', to_rgb(inverse_data_transform(x_orig).squeeze().permute(1, 2, 0).detach().cpu().numpy()))
...
for epoch in tqdm(range(epoch_num)):
    ...
    if psnr > psnr_best:
        psnr_best = psnr
        plt.imsave(best_save_dir + '/epoch_%03d_%.3f.png' % (epoch, psnr_best), to_rgb(img_pred))
    if ssim > ssim_best:
        ssim_best = ssim
    if sam < sam_best:
        sam_best = sam
    ...
    if epoch % save_interval == 0:
        plt.imsave(result_save_dir+'/epoch_%03d_%.3f.png'%(epoch, psnr), to_rgb(img_pred))
