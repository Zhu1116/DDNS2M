...
class VS2M(object):
    def __init__(self, rank, image_noisy, image_clean, beta, num_iter, lr=0.001):
        ...
        self.post = None
        self.dataType = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor
        self._init_all()
        self.out_avg = 0
    ...
    def _init_images(self):
        ...
        image = self.image
        self.image_torch = np_to_torch(image).type(self.dataType)
    ...
    def _init_nets(self):
        ...
        for i in range(self.rank):
            net = skip(self.input_depth, self.output_depth,
    ...
    def _init_inputs(self):
        original_noise = torch_to_np(get_noise1(1, 'noise', (self.input_depth, self.image_clean.shape[0], self.image_clean.shape[1]), noise_type='u',
                                                                     var=10/10.).type(self.dataType).detach())
        self.image_net_inputs = np_to_torch(original_noise).type(self.dataType).detach()[0, :, :, :]


        original_noise = torch_to_np(get_noise2(1, 'noise', self.image.shape[1], noise_type='u', var=10/ 10.).type(self.dataType).detach())
        self.mask_net_inputs = np_to_torch(original_noise).type(self.dataType).detach()[0, :, :, :]
    ...
    def optimize(self, image_noisy, image_clean, at, mask, iteration, logger, avg, update):
        ...
        for j in range(self.num_iter + 1):
            ...
            if update:
                ...
                break
        return self.current_result.recon, self.total_loss, self.best_result.recon, self.best_result.psnr
