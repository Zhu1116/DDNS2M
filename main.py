...
"""
sr:--ni --config celeba_hq.yml --path_y celeba_hq --eta 0.85 --deg "sr_bicubic" --deg_scale 4 --sigma_y 0.1 -i demo --add_noise
de:--ni --config celeba_hq.yml --path_y celeba_hq --eta 0.85 --deg "denoising" --sigma_y 0.2 -i demo --add_noise
in:--ni --config celeba_hq.yml --path_y celeba_hq --eta 0.85 --deg "inpainting" --sigma_y 0.1 -i demo --add_noise
"""
...
def main():
    ...
    # run = wandb.init(project="DIP-project", group="DDNM-denoising0.2", name="denoising0.2")
    args, config, logger = parse_args_and_config()

    try:
        runner = Diffusion(args, config)
        runner.sample(args.simplified, logger)
    except Exception:
