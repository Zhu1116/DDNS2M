...
from functions.svd_operators import SRConv, Denoising, Inpainting
import scipy
import os
from tools import *
...
# run = wandb.init(project="paper11", group="DHP", name="denoising0.2")
...
# 数据参数
data_path = 'exp/datasets/pavia_inpainting.mat'
image_size = 256
channels = 102
...
if deg == 'sr_bicubic':
    ...
    mask = torch.from_numpy(np.float32(mat['mask'])).permute(2, 0, 1).unsqueeze(0)  # (256, 256, 32) --->（1，32，256，256）
    ...
    A_funcs = Inpainting(channels, image_size, missing, keeping, device)
...
z = torch.randn(Apy.shape) * .1
z = z.to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=lr)

for epoch in tqdm(range(epoch_num)):
    img_pred = model(z)
    loss = torch.nn.functional.mse_loss(img_pred * 2 - 1.0, Apy)
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()

    img_pred = img_pred.squeeze().permute(1, 2, 0).detach().cpu().numpy()
    ...
    if psnr > psnr_best:
        ...
        plt.imsave(best_save_dir + '/epoch_%04d_%.3f.png' % (epoch, psnr_best), best)
    if ssim > ssim_best:
        ssim_best = ssim
    if sam < sam_best:
        sam_best = sam
    print('epoch: %04d, loss: %.4f, psnr: %.3f, psnr_best: %.3f, ssim: %.3f, ssim_best: %.3f, sam: %.3f, sam_best: %.3f'
    ...
    if epoch % save_interval == 0:
        ...
        plt.imsave(result_save_dir+'/epoch_%04d_%.3f.png'%(epoch, psnr), save)
